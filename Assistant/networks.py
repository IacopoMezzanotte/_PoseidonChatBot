...
from tensorflow.keras.layers import Dense, Embedding, Flatten, GlobalAveragePooling1D, GlobalMaxPooling1D, MaxPooling1D, \
    Dropout, Conv1D, Input, concatenate
...
def createmodel(training_labels, training_sentences):
    ...
    # creating the model
    inputs = Input(20)
    out = Embedding(vocab_size, embedding_dim, input_length=max_len)(inputs)
    out1 = Conv1D(30, 1, activation="relu")(out)
    out2 = Conv1D(30, 2, activation="relu")(out)
    out3 = Conv1D(30, 3, activation="relu")(out)
    concat = concatenate([out1, out2, out3], axis=1)
    ...
    model.add(Conv1D(48, 6, activation='relu'))
...
def training(model, padded_sequences, val_padded_sequences, training_labels, val_labels):
    ...
    history = model.fit(padded_sequences, np.array(training_labels), epochs=epochs,
                        validation_data=(val_padded_sequences, np.array(val_labels)), callbacks=[early_stop])
